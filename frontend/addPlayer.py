...
class AddPlayer:
  def runSave(self):
    ...
    result = addPlayerToTeam(self.team_id,self.fname.get(),self.lname.get(),type,"")
    if result:
      messagebox.showinfo(" Message","Player Added Successfully!")
      self.master.destroy()
    else:
      messagebox.showerror(" Incomplete Details","Fill Form Completely!")
  ...
  def __init__(self,master, t_id):
      ...
      self.fNameEntry = Entry(self.master,font=('Yu Gothic', 20),textvariable=self.fname, background='#d9d9d9', relief='flat')
      self.fNameEntry.place(x=470, y=318, width=220)

      self.lNameEntry = Entry(self.master,font=('Yu Gothic', 20),textvariable=self.lname, background='#d9d9d9', relief='flat')
      ...
      self.teamSelect = Message(self.master,width=60, font=('Yu Gothic', 15), background="#d9d9d9")
