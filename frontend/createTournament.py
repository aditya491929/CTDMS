...
class CreateTournament:
    def addTour(self):
        res = addTournament(self.tourName.get(), self.host.get(), self.year.get(
        ), self.prize.get(), self.startDate.get(), self.adminId)
        if res:
    ...
    def __init__(self, master, adminId):
        ...
        self.adminIdMessage = Message(self.master, font=(
            'Yu Gothic', 22), text=adminId, background='#d9d9d9', relief='flat')
        self.adminIdMessage.place(x=680, y=240, width=250)

        self.tourNameEntry = Entry(self.master, font=(
            'Yu Gothic', 22), textvariable=self.tourName, background='#d9d9d9', relief='flat')
        self.tourNameEntry.place(x=400, y=330, width=250)

        self.tourIdEntry = Entry(self.master, font=(
            'Yu Gothic', 22), textvariable=self.tourId, background='#d9d9d9', relief='flat')
        self.tourIdEntry.place(x=1070, y=330, width=250)

        self.startDateEntry = Entry(self.master, font=(
            'Yu Gothic', 22), textvariable=self.startDate, background='#d9d9d9', relief='flat')
        self.startDateEntry.place(x=400, y=427, width=250)

        self.yearEntry = Entry(self.master, font=(
            'Yu Gothic', 22), textvariable=self.year, background='#d9d9d9', relief='flat')
        self.yearEntry.place(x=1070, y=427, width=250)

        self.hostEntry = Entry(self.master, font=(
            'Yu Gothic', 22), textvariable=self.host, background='#d9d9d9', relief='flat')
        self.hostEntry.place(x=400, y=527, width=250)

        self.prizeEntry = Entry(self.master, font=(
            'Yu Gothic', 22), textvariable=self.prize, background='#d9d9d9', relief='flat')
        self.prizeEntry.place(x=1070, y=527, width=250)

        self.numMatchesEntry = Entry(self.master, font=(
            'Yu Gothic', 20), textvariable=self.numMatches, background='#d9d9d9', relief='flat')
