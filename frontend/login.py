...
class loginWindow:
    ...
    def __init__(self, master):
        ...
        # these two variables store the input givent via the entry widget
        self.adU = tk.StringVar()
        self.adP = tk.StringVar()

        # these two variables store the input givent via the entry widget
        self.TU = tk.StringVar()
        self.TP = tk.StringVar()

        self.adminUser = Entry(self.master, font=(
            'Yu Gothic', 20), textvariable=self.adU, background='#ebebeb', relief='flat')
        self.adminUser.place(x=220, y=403)
        self.adminPass = Entry(self.master, font=(
            'Yu Gothic', 20), textvariable=self.adP, background='#ebebeb', relief='flat', show='*')
        self.adminPass.place(x=220, y=512)

        self.teamUser = Entry(self.master, font=(
            'Yu Gothic', 20), textvariable=self.TU, background='#ebebeb', relief='flat')
        self.teamUser.place(x=867, y=403)
        self.teamPass = Entry(self.master, font=(
            'Yu Gothic', 20), textvariable=self.TP, background='#ebebeb', relief='flat', show='*')
