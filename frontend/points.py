...
class PointsTable:
    ...
    def __init__(self, master, tour_id):
        ...
        self.table.tag_configure('oddrow',background="#d9d9d9")
        self.table.tag_configure('evenrow',background="#efefef")
        ...
        for i in range(len(self.ptsTable)):
            if i%2 == 0:
                self.table.insert(parent='', index='end', iid=i, values=(i+1, self.ptsTable[i][1], self.ptsTable[i][2], self.ptsTable[i][3],
                                                                     self.ptsTable[i][4], self.ptsTable[i][5]),tags=('evenrow',))
            else:
                self.table.insert(parent='', index='end', iid=i, values=(i+1, self.ptsTable[i][1], self.ptsTable[i][2], self.ptsTable[i][3],
                                                                     self.ptsTable[i][4], self.ptsTable[i][5]),tags=('oddrow',))

        self.table.place(x=130, y=220)

        self.backBtn = Button(self.master, width=10, background='#caf6ff', relief='flat',
                              text='Back', font=('Yu Gothic', 18, 'bold'), command=self.back)
