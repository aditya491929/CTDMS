...
class ViewResult:
    ...
    def __init__(self, master, team_id):
        ...
        self.scrlbar = ttk.Scrollbar(
            self.result, orient="vertical", command=self.result.yview)
        ...
        self.result.tag_configure('evenrow', background="#efefef")
        self.result.tag_configure('oddrow', background="#d9d9d9")

        self.matchResults = matchResult(self.t_id)
        for i in range(len(self.matchResults)):
            if i%2 == 0:
                self.result.insert(parent='', index='end', iid=i, values=(self.matchResults[i][0], self.matchResults[i][1],
                                                                        self.matchResults[i][2], self.matchResults[i][3], 
                                                                        self.matchResults[i][4], self.matchResults[i][5], 
                                                                        self.matchResults[i][6], self.matchResults[i][7]), tags=('evenrow',))
            else:
                self.result.insert(parent='', index='end', iid=i, values=(self.matchResults[i][0], self.matchResults[i][1],
                                                                        self.matchResults[i][2], self.matchResults[i][3], 
                                                                        self.matchResults[i][4], self.matchResults[i][5], 
                                                                        self.matchResults[i][6], self.matchResults[i][7]), tags=('oddrow'))
