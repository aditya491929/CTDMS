...
def getTeamHomeGrounds():
    names = teamIDs()
    home_grounds = Database.getColumnsOf(
        table_name="team", columns=["home_ground"])
...
def getTeamIDMapping():
    result = Database.getColumnsOf(table_name="team", columns=[
                                   "team_name", "team_id"])
    mapping = {}
    for r in result:
        mapping[r[0]] = r[1]
...
def getPlayerId(name):
    query = "SELECT p_id FROM player_info WHERE first_name = '{}'".format(
        name.split(" ")[0])
...
def getTeamPlayers(team_id):
    query = "SELECT player_info.p_id,first_name,last_name,player_type FROM player_info INNER JOIN  player ON player.p_id = player_info.p_id WHERE player.team_id = {}".format(
        team_id)
...
def addTournament(name, host, year, prize_money, startDate, adminId):
    ...
    schedule = matchScheduler(
        new_tournamentId, new_matchId, startDate, "16:00:00.0", venues, teams)
    ...
    values = (new_tournamentId, name, host, year,
              numberOfMatches, prize_money, None, adminId)
...
def getMatchesForTeam(team_id):
    query = "SELECT * from matches WHERE home_team = {t_id} OR away_team = {t_id}".format(
        t_id=team_id)
...
def getPointsTable(tournament_id):
    ...
    query = "SELECT * FROM points_table WHERE tournament_id = {} ORDER BY points DESC".format(
        tournament_id)
...
def playersInMatch(match_id):
    ...
    query = "SELECT home_team,away_team FROM matches WHERE m_id = {}".format(
        match_id)
    result = Database.runQuery(query)[0]
...
def addPlayerToTeam(team_id, first_name, last_name, type_, email):
    ...
    query2 = """
             ...
             VALUES (%s,%s,%s,%s,%s)
             """
    values = (p_id, first_name, last_name,
              first_name[:2]+"."+last_name+"@gmail.com", None)
...
def matchResult(team_id):
    ...
    query = "SELECT * FROM match_summary_view WHERE home_team = '{}' OR away_team = '{}'".format(
        idToName[int(team_id)], idToName[int(team_id)])
    result = Database.runQuery(query)
